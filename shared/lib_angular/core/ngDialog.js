(function (m) {
   ...
   * updated by tianyingchun, causeof ngDialog has some bugs while you popup multiple
   * dialog one time, but in our project, we may encounter with multiple async request.
   *
   */
  ...
  var animationStartEvent = 'animationstart webkitAnimationStart mozAnimationStart MSAnimationStart oanimationstart';
    ...
    this.$get = ['$document', '$templateCache', '$compile', '$q', '$http', '$rootScope', '$timeout', '$window', '$controller',
      function ($document, $templateCache, $compile, $q, $http, $rootScope, $timeout, $window, $controller) {
        ...
        var privateMethods = {
          ...
          calculatePosition: function ($dialog) {
            $dialog.unbind(animationStartEvent);
            var $content = $dialog[0].querySelector('.ngdialog-content');
            var height = $content.offsetHeight;
            // var viewportHeight = window.innerHeight || document.body.clientHeight;
            var top = Math.ceil(height / 2);
            $content.style.marginTop = -top - 10 + 'px';
          },
          ...
          closeDialog: function ($dialog, value) {
            // if we passed dialog id, we need to convert to object.
        ...
        var publicMethods = {
          ...
          open: function (opts) {
            ...
            $q.when(loadTemplate(options.template || options.templateUrl)).then(function (template) {
              ...
              self.$result = $dialog = $el('<div id="ngdialog' + currDialogId + '" class="ngdialog"></div>');
              ...
              // FIXME. maybe we need to remove $timeout.
              // $timeout(function() {
              ...
              // binding animating end events.
              $dialog.bind(animationStartEvent, function (e) {
                privateMethods.calculatePosition($dialog);
              });

              if (options.closeByNavigation) {
                $rootScope.$on('$locationChangeSuccess', function () {
                  if (scopes[self.latestID]) {
                    privateMethods.closeDialog(self.latestID /*$dialog*/ );
                  }
                });
              }
